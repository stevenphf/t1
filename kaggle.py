...
with tf.name_scope('Input'):
    x = tf.placeholder(tf.float32, [None, 784], name='in_x')
    y = tf.placeholder(tf.float32, [None, 10], name='in_y')
    keep_prob = tf.placeholder(tf.float32, name='in_prob')
...
with tf.Session() as sess:
    ...
    # add by steven
    writer = tf.summary.FileWriter("/home/stevenpan/work/tf/tk_log", sess.graph)
    ...
    for epoch in range(5):
        ...
        # 每次准确度评估  validation_images和validation_labels数据要从train数据中预留，待加
        # accuracy_n = sess.run(accuracy,feed_dict={x:validation_images,y:validation_labels,keep_prob:1.0})
        # print('epoch '+ str(epoch+1)+'准确度： '+str(accuracy_n))

    # 保存训练模型
    saver.save(sess, 'ModelconvNN2/model.ckpt')

    # 验证
    saver.restore(sess, 'ModelconvNN2/model.ckpt')
    validation_size = 2000
    validation_x = x_test[:validation_size]
    validation_y = labels[:validation_size]
    accuracy_n = sess.run(accuracy, feed_dict={x: validation_x, y: validation_y, keep_prob: 1.0})
    print('验证准确度：\t' + str(accuracy_n))
